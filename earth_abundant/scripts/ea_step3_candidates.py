...
Earth-abundant dopants targeted:
  Li-site (24d, val +3):  FeÂ³âº, AlÂ³âº, MgÂ²âº, MnÂ³âº, ZnÂ²âº, TiÂ³âºâ†’(Li-site rare, use Zr-site for Ti)
  Zr-site (16a, val +4):  Tiâ´âº, Mnâ´âº, Feâ´âº (uncommon), Nbâµâº(abundant ore)
...
Physics checks applied:
  1. Charge neutrality â†’ Li_pfu range 6.0â€“6.9
  2. Li_pfu in the superionic window (6.1â€“6.8)
  3. Concentration caps per known stability limits
  4. Shannon ionic radius compatibility (dopant fits the site)

Validation pipeline:
  CHGNet  â†’ energy/atom + volume (static predict_structure)
  M3GNet  â†’ independent energy cross-check
  Cross-check: Î”E between two models < 0.15 eV/atom â†’ accepted
...
# ROOT = d:/doped_2 (two levels up from earth_abundant/scripts/)
ROOT = Path(__file__).parent.parent.parent
CWD  = Path(__file__).parent

# â”€â”€â”€ Dopant definitions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

LI_SITE_DOPANTS = {
    # element: (valence, max_fraction, cost_tier, note)
    "Fe": (3, 0.20, "low",  "FeÂ³âº on Li-site; above x=0.20 causes electronic conductivity"),
    "Al": (3, 0.30, "low",  "AlÂ³âº on Li-site; best known Li-site dopant, very cheap"),
    "Mg": (2, 0.25, "low",  "MgÂ²âº on Li-site; aliovalent, creates extra vacancies"),
    "Mn": (3, 0.15, "low",  "MnÂ³âº/4+ mixed; risk of reduction to MnÂ²âº at high conc."),
    "Zn": (2, 0.20, "low",  "ZnÂ²âº on Li-site; less studied, potentially novel"),
}

ZR_SITE_DOPANTS = {
    # element: (valence, max_fraction, cost_tier, note)
    "Ti": (4, 0.50, "low",  "Tiâ´âº isovalent; expands lattice, enhances bottleneck"),
    "Nb": (5, 0.50, "low",  "Nbâµâº donor; ore-abundant (columbite), most studied Zr-site"),
    "Mn": (4, 0.30, "low",  "Mnâ´âº on Zr-site; novel, earth-abundant"),
    "Fe": (4, 0.20, "low",  "Feâ´âº on Zr-site (rare oxidation state); less common"),
    "Sn": (4, 0.30, "low",  "Snâ´âº isovalent; lattice softening, novel in LLZO"),
}
...
# â”€â”€â”€ Charge balance â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def compute_li_pfu(li_val, x, zr_val, y):
    ...
    Sum of cation charges = 24 (to balance 12 OÂ²â»):
      nÃ—(+1) + xÃ—li_val + 3Ã—(+3) + (2-y)Ã—(+4) + yÃ—zr_val = 24
      n = 24 - xÃ—li_val - 9 - (2-y)Ã—4 - yÃ—zr_val
    """
...
# â”€â”€â”€ Candidate generation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def generate_candidates():
    ...
    for (li_el, (li_val, li_max, li_tier, li_note)), \
        ...
        # Skip Mn+Mn (same element on both sites â†’ ordering impossible)
        if li_el == zr_el:
            continue
    ...
    print(f"Li_pfu range: {df['Li_pfu'].min():.3f} â€“ {df['Li_pfu'].max():.3f}")
    return df


# â”€â”€â”€ Base structure builder â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def build_substituted_structure(base_struct, li_el, x_frac, zr_el, y_frac):
    ...
    Replaces atoms deterministically (ordered substitution):
      â€¢ Li-site dopant replaces proportional fraction of Li (24d sites)
      â€¢ Zr-site dopant replaces proportional fraction of Zr (16a sites)
    """
...
# â”€â”€â”€ CHGNet validation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€ M3GNet validation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def validate_with_m3gnet(structure):
    try:
        ...
        print("  âš   matgl not installed â€” skipping M3GNet cross-check")
        return None
    except Exception as e:
        print(f"  âš   M3GNet error: {e}")
        return None


# â”€â”€â”€ Main screening loop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def run_validation(df, max_candidates=20):
    """
    Validates up to max_candidates from the pool using CHGNet + M3GNet.
    Filters: |Î”E(CHGNet - M3GNet)| < 0.15 eV/atom â†’ accepted
    """
    from chgnet.model import CHGNet
    print("\nâ”€â”€ Loading CHGNet â”€â”€")
    ...
    for idx, row in subset.iterrows():
        ...
        try:
            ...
            print(f"  CHGNet: E/atom={e_chgnet:.4f} eV, Vol/atom={vol_pa:.3f} Ã…Â³")
            ...
            if e_m3gnet is not None:
                ...
                print(f"  M3GNet: E/atom={e_m3gnet:.4f} eV  |Î”E|={delta_e:.4f} â†’ {'âœ… PASS' if cross_ok else 'âŒ FAIL'}")
            else:
                delta_e = None
                cross_ok = None
...
def main():
    out_raw = ROOT / "earth_abundant" / "data" / "candidates" / "earth_abundant_candidates_raw.csv"
    out_val = ROOT / "earth_abundant" / "data" / "candidates" / "earth_abundant_candidates_validated.csv"
