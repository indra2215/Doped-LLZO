...
def main():
    ...
    print(f"Base structure has {len(base_structure)} atoms.", flush=True)

    cif_folder = Path("cif")
    cifs = sorted(cif_folder.glob("*.cif"))
    
    for cif_path in cifs:
        formula = cif_path.name.replace("_evaluated.cif", "").replace(".cif", "")
        print(f"\nProcessing {formula}...", flush=True)
        
        try:
            modified_struct = build_substituted_structure(base_structure, formula)
            print(f"Substituted atoms: {len(modified_struct)}", flush=True)
            
            print("Relaxing positions (10 steps)...", flush=True)
            res1 = optimizer.relax(modified_struct, relax_cell=False, fmax=0.5, steps=10, verbose=False)
            pos_relaxed = res1['final_structure']
            
            print("Relaxing full cell (10 steps)...", flush=True)
            ...
            print(f"Final vol/atom: {vol_per_atom:.2f} A^3", flush=True)
            
            final_struct.to(fmt="cif", filename=str(cif_path))
            print("Saved CIF.", flush=True)
